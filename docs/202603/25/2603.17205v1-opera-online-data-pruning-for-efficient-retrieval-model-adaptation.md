...
tldr: "针对稠密检索模型在特定领域微调时训练数据质量不一的问题，本文提出OPERA框架。研究发现静态剪枝虽能提升排序性能但会损害召回率，为此作者提出两阶段动态剪枝（DP）策略，在训练过程中自适应调节查询和文档级的采样概率。实验证明，DP在显著提升检索和排序性能的同时，将训练时间缩短了50%以上，且在LLM检索器上表现出良好的通用性。"
selection_source: fresh_fetch
motivation: 旨在解决稠密检索模型微调中不同训练样本贡献不均，以及静态剪枝导致的排序性能与召回率之间的权衡问题。
method: 提出一种两阶段动态剪枝策略，通过在训练过程中自适应地调节查询和文档层面的采样概率，优先处理高质量样本并保持数据多样性。
result: "动态剪枝在多个领域数据集上均取得最优表现，NDCG@10提升1.9%，且仅需不到50%的训练时间即可达到标准微调的效果。"
conclusion: OPERA框架通过在线数据剪枝有效平衡了检索模型的训练效率与效果，是一种架构无关的高效模型适配方案。
---
