...
## 速览
**TLDR**：本研究开发了TorchNWP编译库工具，旨在解决Fortran数值模型与Python深度学习框架之间的跨语言兼容性和耦合效率问题。该工具基于LibTorch，通过统一的C/C++接口将PyTorch模型集成到数值模型中，并实现了神经网络的切线性与伴随模型。TorchNWP支持异构平台和高效并行，能以极低的代码修改成本实现深度学习组件与CMA-GFS等气象预报系统的深度融合，显著提升了数值天气预报的精度与效率。 \
**Motivation**：解决传统Fortran数值模型与现代Python深度学习框架之间跨语言兼容性差、耦合灵活性不足及数据传输效率低的问题。 \
**Method**：基于LibTorch开发TorchNWP工具，通过统一的C/C++接口和混合编程技术，在底层实现神经网络的切线性与伴随模型。 \
**Result**：成功将深度学习物理参数化方案集成至CMA-GFS等数值预报模型中，并实现了高效的四维变分数据同化构建。 \
**Conclusion**：TorchNWP显著降低了AI与数值模型的耦合成本，为提升数值天气预报的准确性和计算效率提供了有效的技术支撑。
