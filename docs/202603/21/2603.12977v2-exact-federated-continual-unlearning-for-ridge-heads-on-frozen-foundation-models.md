...
title_zh: 冻结基础模型上岭回归头的精确联邦持续遗忘学习
...
motivation: 现有的联邦遗忘方法通常只能实现近似遗忘或面临极高的重训成本，难以在保证精确性的同时处理持续的删除请求。
method: 利用岭回归的加性充分统计量构建通信协议，通过固定大小的消息同步增删操作，确保服务器模型在算术上与集中式重训完全等效。
result: 实验显示该方法在多个基准上达到了 10^-9 级别的相对误差，且处理请求的效率比联邦重训基线高出数个数量级。
conclusion: 本文证明了在特定模型架构下可以实现低成本且确定性精确的联邦持续遗忘，并提供了贝叶斯零散度证明及多种不变性保证。
---

## 摘要
基础模型通常被部署为冻结的特征提取器，并带有一个小型可训练头，以便在联邦设置中适应私有的用户生成数据。“被遗忘权”要求根据需求从已训练的模型中移除特定样本或用户的影响。现有的联邦遗忘学习方法针对通用的深度模型，并依赖于近似重构或选择性重训练，这使得精确性要么成本高昂，要么难以实现。我们在一个具有实际意义但尚未得到充分探索的领域研究这一问题：带有岭回归头的冻结基础模型。精确的最优解仅通过两个加性充分统计量依赖于数据，我们将这些统计量转化为一种通信协议，通过固定大小的消息支持任意的添加和删除请求流。服务器维护一个模型头，在精确算术下，该头在每次请求后与中心化重训练的结果在逐点意义上完全一致。我们提供了确定性的重训练等效性保证、顺序和分区不变性、两种服务器端变体，以及零 KL 散度的贝叶斯证明。在四个基准测试上的实验证实了这些保证：两种变体与中心化岭回归重训练的相对 Frobenius 误差均在 10^-9 以内，并且完成每次请求的成本比联邦重训练基准低几个数量级。
