...
date: 2026-02-25
pdf: "https://arxiv.org/pdf/2602.21897v2"
tags: ["query:mlsys"]
score: 8.0
evidence: 异构系统与加速器API的编程方法论
tldr: 针对异构计算系统中多种加速器API（如CUDA、SYCL）难以协同工作的问题，本文提出一种基于任务的数据流方法。通过引入任务感知库（TA-libs）如TASYCL和TACUDA，将加速器调用提升为一级任务，并利用nOS-V库统一线程管理。该方法简化了多API集成，解决了线程竞争问题，实现了在单一应用中透明且高效地利用多种加速器资源。
selection_source: carryover_cache
motivation: 异构系统中多种底层加速器API共存导致编程复杂、同步困难且存在线程竞争导致的性能波动。
method: 采用基于任务的数据流模型，结合任务感知库（TASYCL/TACUDA）和nOS-V线程管理库，将加速器内核封装为由运行时管理的有向无环图任务。
result: 实验证明该方法能让单一应用程序透明、高效地利用多种加速器编程模型，并有效解决了多运行时环境下的线程超卖问题。
conclusion: 任务感知库与统一线程管理相结合的方案，为当前及未来复杂的异构计算系统提供了一种可扩展且高效的编程范式。
---
