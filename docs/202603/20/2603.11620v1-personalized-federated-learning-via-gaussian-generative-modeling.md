...
tags: ["query:mlsys"]
score: 7.0
evidence: 在分布式客户端数据上进行协作训练
tldr: 针对联邦学习中的数据异构性挑战，本文提出了一种基于高斯生成建模的个性化联邦学习方法 pFedGM。该方法通过高斯生成器建模客户端异构性，并采用双重目标优化类间与类内距离。通过将全局分布视为先验、本地数据视为似然，利用贝叶斯推理和双尺度融合框架为每个客户端生成个性化分类器，在多种异构场景下均取得了优于现有技术的效果。
selection_source: fresh_fetch
motivation: 现有个性化联邦学习方法主要关注分类器引导，忽视了表示分布中潜在的个性化特征。
method: 提出 pFedGM 方法，利用高斯生成建模和双尺度融合框架，结合全局先验与本地似然进行贝叶斯推理以构建个性化分类器。
result: 在类别数量异构、环境损坏等多种基准测试和配置下，pFedGM 均达到了领先或极具竞争力的性能。
conclusion: 通过建模表示分布的个性化特征并平衡全局协作与本地特性，高斯生成建模能有效提升个性化联邦学习的性能。
---
