...
## 摘要
训练后量化对于在资源受限的硬件上部署大语言模型（LLM）至关重要，但目前最先进的方法在各层之间强制执行统一的位宽，导致准确率与效率之间的权衡并非最优。我们提出了 RAMP（强化自适应混合精度），这是一个离策（off-policy）Soft Actor-Critic 框架，它学习逐层位宽分配，以在全局比特预算下最小化困惑度。该策略以激活统计量、权重属性和结构描述符的 11 维嵌入为条件，实现了跨模型系列和规模的零样本迁移。为了实现稳定的 4 比特以下量化，我们引入了 Scale Folding，这是一种预处理技术，通过逐通道缩放和归一化层补偿将激活离群值迁移到权重中。具有非对称惩罚和预算悬崖的质量优先奖励机制驱动了快速收敛。在 Llama 2 7B 上，RAMP 在 3.68GB（3.65 有效比特）下实现了 5.54 的困惑度，在模型大小上优于统一 4 比特 AWQ（3.90 GB 下为 5.60）和 GPTQ 6%，在质量上优于 1% 至 3%。至关重要的是，仅在 Llama 2 7B 上训练的策略可以零样本泛化到 Llama 2 13B 和 Mistral 7B，且通常优于针对特定目标的训练，这支持了量化敏感性主要由架构决定的假设。HALO 流水线将分配方案导出为 GGUF 格式，以便在 CPU、GPU 和边缘设备上进行无算子推理，保留了 99.5% 的 FP16 常识推理性能。
