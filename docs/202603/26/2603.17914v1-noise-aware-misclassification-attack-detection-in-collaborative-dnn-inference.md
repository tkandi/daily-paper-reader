...
tags: ["query:mlsys"]
score: 6.0
evidence: 协作式DNN推理与边缘卸载架构
tldr: "本研究针对边缘AI中协作式DNN推理面临的恶意数据注入攻击，提出了一种噪声感知的异常检测框架。由于环境噪声常导致误分类攻击难以检测，该方案采用变分自编码器（VAE）构建半灰盒模型，通过提取鲁棒的噪声感知特征来捕捉对抗性篡改引起的偏差。实验表明，该方法在多种DNN配置下可达到90%的AUROC，显著提升了检测精度并降低了误报率，为保障协作推理安全提供了有效手段。"
selection_source: fresh_fetch
motivation: 协作式DNN推理易受恶意数据注入攻击，且环境噪声的存在使得隐蔽的误分类攻击极难被准确检测。
method: 提出一种基于变分自编码器（VAE）的半灰盒检测框架，通过引入噪声感知特征来区分环境噪声与对抗性篡改。
result: "在主流目标分类DNN上的评估显示，该框架在现实噪声条件下实现了高达90%的AUROC，有效降低了误报率。"
conclusion: 该方案证明了噪声感知特征在提升攻击检测鲁棒性方面的有效性，同时也指出了特征相似性和极高噪声对检测的局限性。
---
