...
tags: ["query:mlsys"]
score: 9.0
evidence: 异构LLM集群的预测性调度系统
tldr: Chimera是一个针对异构大模型集群的多智能体工作流预测调度系统。针对现有系统在处理多阶段任务时忽视模型异构性带来的性能与延迟权衡问题，Chimera通过语义路由评估模型置信度、预测工作流剩余长度并结合负载均衡策略，实现了端到端延迟的显著降低和任务性能的提升，为复杂智能体应用的高效部署提供了新方案。
selection_source: fresh_fetch
motivation: 现有的LLM推理系统多假设同构集群，忽视了异构模型在处理多阶段智能体工作流时在延迟与性能权衡上的潜力。
method: Chimera利用语义路由预测模型置信度，并结合工作流剩余长度预测与实时负载估算，实现跨异构模型的智能请求调度。
result: 在代码生成和数学推理任务中，Chimera相比vLLM等基准将端到端延迟降低了1.2-2.4倍，同时将任务性能平均提升了8.0-9.5个百分点。
conclusion: Chimera通过协同优化延迟与性能，证明了在异构集群上进行预测性调度是提升多智能体工作流服务效率的有效途径。
---
