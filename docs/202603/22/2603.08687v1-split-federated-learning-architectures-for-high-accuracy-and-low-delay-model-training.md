...
tldr: 本研究针对分层拆分联邦学习（HSFL）中模型切分层和客户端分配对性能影响不明的问题，提出了一种新型网络架构优化方案。通过建立联合优化模型，在保证高准确率的同时降低训练延迟和通信开销。研究证明了该问题的NP难特性，并设计了一种感知准确率的启发式算法。实验表明，该方法在提升准确率的同时，显著减少了延迟和开销，为高效分布式模型训练提供了新思路。
selection_source: carryover_cache
motivation: 现有的分层拆分联邦学习忽略了模型切分位置和客户端分配对训练准确率、延迟及通信开销的综合影响。
method: 将切分层选择与客户端到聚合器的分配建模为联合优化问题，并提出一种兼顾准确率与延迟效率的启发式算法。
result: "实验显示，该方法相比现有方案使准确率提升了3%，同时延迟降低了20%，通信开销减少了50%。"
conclusion: 通过优化网络架构中的切分层和资源分配，可以同时实现高精度、低延迟和低开销的联邦模型训练。
---

## 摘要
我们能否在拆分联邦学习（SFL）中找到一种用于机器学习模型训练的网络架构，以优化训练损失（从而提高精度）？这种架构是否也能减少训练延迟和通信开销？虽然在普通的、最先进的 SFL 中，精度不受模型拆分方式的影响，但在本研究中，我们对上述问题给出了肯定的回答。最近的分层 SFL（HSFL）架构采用了由客户端、（本地）聚合器和中央服务器组成的三层训练结构。在该架构中，模型在两个拆分层处被划分为三个子模型，并在三个层级上执行。尽管 HSFL 架构具有诸多优点，但它忽略了拆分层和客户端到聚合器的分配对精度、延迟和开销的影响。本研究通过构建联合优化问题，明确捕捉了拆分层和客户端到聚合器分配对精度、延迟和开销的影响。我们证明了该问题是 NP 难的，并提出了首个感知精度的启发式算法，该算法在保持延迟效率的同时，明确考虑了模型精度。在公共数据集上的仿真结果表明，与最先进的 SFL 和 HSFL 方案相比，我们的方法可以将精度提高 3%，同时减少 20% 的延迟和 50% 的开销。
