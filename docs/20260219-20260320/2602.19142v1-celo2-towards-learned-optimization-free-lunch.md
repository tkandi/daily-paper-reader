...
title_zh: Celo2：迈向学习优化的免费午餐
authors: "Abhinav Moudgil, Boris Knyazev, Eugene Belilovsky"
date: 2026-02-22
pdf: "https://arxiv.org/pdf/2602.19142v1"
tags: ["query:mlsys"]
score: 8.0
evidence: 通过学习型优化器优化训练过程
tldr: 学习优化器虽强但面临元训练成本高和泛化性差的挑战。本文提出Celo2，通过简化的归一化架构和增强元训练，仅需4.5个GPU小时即可完成训练。该优化器在13亿参数的GPT-3 XL预训练任务上表现稳定，泛化能力跨越六个数量级，且兼容现代优化技术，为实用化学习优化算法铺平了道路。
selection_source: fresh_fetch
motivation: 解决现有学习优化器元训练成本极高且难以泛化到大规模模型和分布外任务的问题。
method: 采用简单的归一化优化器架构，并结合增强的元训练策略与现代优化技术（如解耦权重衰减）。
result: 仅用4.5个GPU小时训练出的模型成功扩展至13亿参数规模，并在多种分布外任务中表现优异。
conclusion: 本研究证明了低成本训练高性能通用学习优化器的可行性，显著降低了该领域的准入门槛。
---

## 摘要
学习优化器是 Adam 等人工设计更新规则的强大替代方案，但由于它们往往难以在训练分布之外进行元泛化，且元训练成本高昂，因此在实际应用中受到限制。例如，先前的工作 VeLO 将元训练规模扩大到 4,000 个 TPU 月（约 GPT-3 计算量的 10 倍）以训练通用优化器，但它无法泛化到超过 6 亿参数的任务。在这项工作中，我们提出了一个令人惊讶的发现：通过构建一个简单的归一化优化器架构并增强元训练，仅需 VeLO 计算量的一小部分（具体为 4.5 个 GPU 小时），即可元训练出一个高性能的通用学习更新规则。我们的学习更新规则可以稳定地扩展到十亿级预训练任务（GPT-3 XL 1.3B），这比其元训练分布大六个数量级。此外，它在各种分布外任务中表现出强大的性能，并与现代优化框架兼容，包括正交化、针对输入输出和隐藏权重的不同更新规则以及解耦权重衰减。总之，这项工作为具有实际应用价值的可学习优化算法铺平了道路，开启了对更丰富的元训练和数据整理方案的探索，以进一步提升性能。
